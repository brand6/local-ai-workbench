...
项目技能管理只保留 `SkillHub技能` 和 `本地技能` 两个页签。`本地技能` 页签按来源分成 `SkillHub`、`Local`、`Plugin` 三块，其中 `Plugin` 块展示完整 plugin 安装带来的技能。`SkillHub技能` 页签可以显示 plugin 安装带来的 skill，但这些行是只读：不能取消 plugin 的 skill，也不能用 SkillHub 安装同名 skill 去覆盖 plugin-owned skill。对应的 AgentHub 项目页签规则放到 AgentHub PRD 中展开。
...
17. As a local AI tool user, I want a `Plugin` source group inside the `本地技能` tab, so that I can inspect the skill surface produced by complete plugins without adding another tab.
...
- Project skill UI tests should cover the `SkillHub技能` / `本地技能` tabs, the `SkillHub` / `Local` / `Plugin` source groups inside `本地技能`, plugin-owned readonly rows, and managed versus existing counts.
